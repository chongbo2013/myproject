...
public class InterpolatorMainActivity extends AppCompatActivity {
    private static final long DURATION = 1200;

    private ListView listView;
    private InterpolatorEaseAdapter adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        super.onCreate(savedInstanceState);
        setContentView(R.layout.interpolator_activity_main);

        init();
        listView = (ListView) findViewById(R.id.list);
        adapter = new InterpolatorEaseAdapter(this, nameList, interpolatorList, DURATION);
