...
public class StatusExpandAdapter extends BaseExpandableListAdapter {
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		if (oneList.get(groupPosition).getTwoList().get(0).isIsfinished()) {
			holder.group_tiao.setBackgroundColor(context.getResources()
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (convertView != null) {
			...
			viewHolder.childName = (TextView) convertView
					.findViewById(R.id.two_status_name);
		...
		if (entity.isIsfinished()) {
			viewHolder.tiao.setBackgroundColor(context.getResources().getColor(
					R.color.yellow));
		} else {
			viewHolder.tiao.setBackgroundColor(context.getResources().getColor(
					R.color.grey));
