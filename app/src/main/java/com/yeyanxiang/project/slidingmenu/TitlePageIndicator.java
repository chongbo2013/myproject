 ...
 * @update 2014年3月13日
 * 
 * @简介 A TitlePageIndicator is a PageIndicator which displays the title of left
 *     view (if exist), the title of the current select view (centered) and the
 *     title of the right view (if exist). When the user scrolls the ViewPager
 *     then titles are also scrolled.
 */
			...
			if ((bound.left > left && bound.left < right)
				...
				if (currentPage && currentSelected) {
					...
					canvas.drawText(pageTitle, 0, pageTitle.length(),
							bound.left + TextMaginLeftAndRight / 2,
							bound.bottom + mTopPadding, mPaintText);
				}
			}
		}
