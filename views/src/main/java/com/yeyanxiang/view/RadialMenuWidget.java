	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (state == MotionEvent.ACTION_DOWN) {
			...
			if (inCircle == true) {
				...
				selected = null;
				showText(centerCircle.getName() + " pressed.");
				...
				for (int i = 0; i < Wedges.length; i++) {
					...
					if (f == selected) {
						...
						// outer ring an
						if (enabled != null) {
							showText("Closing outer ring");
							enabled = null;
							animateOuterIn = true; // sets Wedge2Shown = false;

							// If outer ring is not enabled, then executes event
						} else {
							showText(menuEntries.get(i).getName() + " pressed.");
				...
				for (int i = 0; i < Wedges2.length; i++) {
					Wedge f = Wedges2[i];
					if (f == selected2) {
						showText(wedge2Data.getChildren().get(i).getName()
								+ " pressed.");
						animateOuterIn = true; // sets Wedge2Shown = false;
						enabled = null;
						selected = null;
				...
				// is selected
				showText("Area outside rings pressed.");
