...
public class RoundAngleImageView extends ImageView {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			roundWidth = a.getDimensionPixelSize(
					R.styleable.RoundAngleImageView_xRadius, roundWidth);
			roundHeight = a.getDimensionPixelSize(
					R.styleable.RoundAngleImageView_yRadius, roundHeight);
			a.recycle();
		} else {
	...
	private void drawLiftDown(Canvas canvas) {
		...
		path.arcTo(new RectF(0, getHeight() - roundHeight * 2, roundWidth * 2,
				getHeight()), 90, 90);
		path.close();
		canvas.drawPath(path, paint);
	}
