 ...
 * @update 2014年4月3日
 * 
 * @简介 本类实现了文字的竖直排版显示（中国古时的行文形式）， 虽然仍有许多特效及功能仍未实现，但基本的使用已经能满足。
 */
public class TextViewVertical extends View {
	...
	/*
	 * //获取整数值 private final int getAttributeIntValue(AttributeSet attrs,String
	 * field) { int intVal = 0; //TODO
	 * //应该可以直接用attrs.getAttributeIntValue()获取对应的数值的，
	 * //但不知道为什么一直无法获得只好临时写个函数凑合着用,没有写完整，暂时只支持px作为单位，其它单位的转换有空再写 String
	 * tempText=attrs.getAttributeValue(androidns, field); intVal =
	 * (int)Math.ceil(Float.parseFloat(tempText.replaceAll("px",""))); return
	 * intVal; }
	 */
	// 设置文字
	...
	protected void onDraw(Canvas canvas) {
		...
		if (drawable != null) {
			// 画背景
			Bitmap b = Bitmap.createBitmap(drawable.getBitmap(), 0, 0,
